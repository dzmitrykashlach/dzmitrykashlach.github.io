...
Обязанности:
- Рефакторинг архитектуры: разработал proxy-микросервис, который устранил зависимость от внутреннего банковского приложения и позволил интегрировать внешних партнёров в кредитный конвейер.  
- Реализовал бизнес-функционал по запросам продукта: автоматические погашения, цессия, интеграция с внешним банком для стабильной обработки погашений.  
- Реализовал несколько контрактов сервисов (REST, Apache Thrift) с unit- и integration-тестами.  
- Реализовал кастомизацию Spring Cloud Consul Agent для перерегистрации микросервиса в случае перезапуска Consul-агента.  
...
- Разработал процесс непрерывного анализа производительности и обучил двух инженеров работе по новому алгоритму.  
