...
&nbsp;&nbsp;Frankly, I'm not using them very often, but occasionally decided to measure performance difference between regular `pow` operation
and bitwise `<<<`, because heard multiple times that bitwise operations perform better.  
...
* the same power of 2 using left unsigned shift
  ...
  {% endhighlight %}  
&nbsp;&nbsp;[The whole benchmarks is here](https://github.com/dzmitrykashlach/algorithms-storehouse/blob/main/benchmarks/src/main/kotlin/bitwise/BitwiseBenchMark.kt)  
...
* `Math.pow()` performs better and show __O(1)__ complexity
* left shift `<<<` is faster then everything above.    
