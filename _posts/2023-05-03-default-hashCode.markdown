...
&nbsp;&nbsp; Here is a sample from JVM `synchronizer.cpp`.  Notice, that if object has no hash, method `get_next_hash(current, obj)`
is called.
{% highlight java %}
...
&nbsp;&nbsp; Happy interviews :-) and do not hesitate to show people
JVM sources, if needed.